...
export const FormattedDecimal: React.FunctionComponent<Props> = ({ value, precision = 6 }) => {
  const theme = useTheme();

  return (
    <FormattedNumberParts value={value} maximumFractionDigits={precision} minimumFractionDigits={precision}>
      {parts => (
        <>
          {parts.map((part, i) => {
            switch (part.type) {
